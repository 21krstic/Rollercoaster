...
unsigned loadImageToTexture(const char* filePath) {
    stbi_set_flip_vertically_on_load(true); // flip public API

    int w, h, ch;
    unsigned char* data = stbi_load(filePath, &w, &h, &ch, 4); // force RGBA
    if (!data) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // možda prebaciti u nearest neighbour?
